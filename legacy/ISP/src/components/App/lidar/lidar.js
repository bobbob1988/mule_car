...
componentWillUpdate(nextProps) {
  ...
  // if(this.props.value.vertices.length !== nextProps.value.vertices.length){
    if(nextProps.value.vertices.length > 0){
     ...
     //console.log('animate')
     //this.setState({vertices: nextProps.value.vertices})
     //this.renderScene()
...
update(pointCloud) {
  ...
  const pointCloudSize = pointCloud.length / 4;
  console.log('length',pointCloudSize)
  const total = (pointCloudSize < 100000) ? pointCloudSize : 100000;
  //console.log('points', this.points)
  ...
  for (let i = 0; i < total; i++) {
    const x = pointCloud[i*4];
    const y = pointCloud[i*4 + 1];
    const z = pointCloud[i*4 + 2];
    positions[i*3] = x;
    positions[i*3 + 1] = y;
